...
/** Parses a comma-separated ID list into a Set for O(1) lookup and exact matching. */
...
export default definePlugin({
    ...
    renderDeleteTimestamp: ErrorBoundary.wrap(({ message: { id: messageId, channel_id: channelId } }: { message: Message; }) => {
        const message = useStateFromStores(
            [MessageStore],
            () => MessageStore.getMessage(channelId, messageId) as MLMessage,
            null,
            (oldMsg, newMsg) => (oldMsg as MLMessage)?.deleteTimestamp === (newMsg as MLMessage)?.deleteTimestamp
        );

        const deleteTs = (message as MLMessage).deleteTimestamp;
        if (!deleteTs) return null;

        return (
            <div className="messagelogger-delete-timestamp">
                <Timestamp
                    timestamp={deleteTs}
                    isEdited={false}
                    isInline={false}
                >
                    <span className={MessageClasses.edited}>{" "}(Deleted)</span>
                </Timestamp>
            </div>
        );
    }, { noop: true }),
    ...
    shouldIgnore(message: any, isEdit = false) {
        try {
            ...
            const userIds = parseIdList(ignoreUsers);
            const channelIds = parseIdList(ignoreChannels);
            const guildIds = parseIdList(ignoreGuilds);
            const msgChannel = ChannelStore.getChannel(message.channel_id);

            return ignoreBots && message.author?.bot ||
                ignoreSelf && message.author?.id === myId ||
                userIds.has(message.author?.id) ||
                channelIds.has(message.channel_id) ||
                channelIds.has(msgChannel?.parent_id) ||
                (isEdit ? !logEdits : !logDeletes) ||
                guildIds.has(msgChannel?.guild_id) ||
                // Ignore Venbot in the support channels
                (message.author?.id === VENBOT_USER_ID && msgChannel?.parent_id === SUPPORT_CATEGORY_ID);
        } catch (e) {
            return false;
        }
    },
    ...
    EditMarker({ message, className, children, ...props }: any) {
        return (
            <TooltipContainer text="Click to view edit history">
                <span
                    {...props}
                    className={classes("messagelogger-edit-marker", className)}
                    onClick={() => openHistoryModal(message)}
                    onKeyDown={e => (e.key === "Enter" || e.key === " ") && openHistoryModal(message)}
                    role="button"
                    tabIndex={0}
    ...
    patches: [
...

        {
            ...
            find: ".SEND_FAILED,",
            replacement: {
                // Render editHistory behind the message content, and deletion timestamp if deleted
                match: /\]:\i.isUnsupported.{0,20}?,children:\[/,
                replace: "$&arguments[0]?.message?.editHistory?.length>0&&$self.renderEdits(arguments[0]),arguments[0]?.message?.deleted&&$self.renderDeleteTimestamp(arguments[0]),"
            }
        },
